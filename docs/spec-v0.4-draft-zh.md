...
>
> 参考校验器只在首行 `v="0.4"` 时检查本文件的语法；`v="0.3"` 的块里出现这些写法时，按 v0.3 规则处理（未知属性、未知行、未知段只警告；已知段写错报错）。样例：`fixtures/blocks/valid/12-v04-stream.txt`、`18-v04-lag-derived.txt`，`fixtures/blocks/invalid/17-v04-rules.txt`、`18-v03-with-v04-syntax.txt`、`24-v04-lag-derived-bad.txt`、`25-v04-stream-bad.txt`。
...
规则（2026-09-18 补，可实现性检查）：

1. `yes` = 生产者在 `reply_end` 之前收到过**至少一个正文片段**（宿主开着流式，而且这条回复确实分片显示了）；`no` = 回复整条一次出现（宿主关了流式，或流式没生效）。
2. 本轮有被读的回复（`gen` 行不是 `n/a`）时生产者**应该**写这个属性；首轮没有回复可读时省略。校验器：v0.4 块里 `gen` 行有时长而首行没有 `stream` 时警告 `STREAM_ATTR_MISSING`。
3. `trigger="continue"` 的重放块原样带原块的值；`swipe` / `regenerate` 按被换掉的那条回复写。
...
stream-line  = %s"stream(lag " 1*DIGIT %s"s): wait " dur [" (" text ")"]
               ...
               " | " hr-part                                  ; 峰值段用 §4 的 peak-part，可带 carryover
...
|---|---|
| `lag Ns` | 与首行 `lag` 相同（§2） |
| `wait` | 从发送（`gen` 的起点）到第一个**正文**字出现的**墙钟**时长（不扣离开），含思维链；括号里可以写 `ttft Ns`、`reasoning Ns`，与 `gen` 行括号里的量相同 |
| `body` | 第一个正文字出现到 `reply_end` 的墙钟时长，以及正文总字数；字数与 `read-pos` 的总字数（`replyChars`）同一口径：只数正文，思维链和 `<bio_act/>` 标签不计 |
| `hr` | 只统计 `body` 区间里在场的秒（§6）；起止值、区间、`cov`、`peak` 的全部规则与相位行相同（v0.3 §1.4、本文件 §2）：`@Ns` 相对 **body 开始**（第一个正文字出现）的在场秒数；`body` 短于 `max(10 s, 2 × lag)`、`cov` < 70%、稀疏来源都不出 `peak`；`@Ns < lag` 必须写 `carryover` |
| `pos` | 时刻 `peakAt − lag`（相对 body 开始）屏幕上已显示的正文字数 / 总字数，以及所在段落 / 总段落数。生产者在每次收到流式片段时记录“时刻 → 已显示字数”，取记录里时刻 ≤ `peakAt − lag` 的**最后一条**（阶梯函数，不插值）；段落 = 该字数落在第几段，分段与总段数和 `read-pos` 同一口径 |
| 其余段 | 同相位行：`cov`、`rr-loss`、`hrv`、`mean`、`above`、`act`（§5.1）、`flag`；都只在 `body` 区间上算 |

规则：

1. 流式时生产者**应该**输出 `stream` 行；只有首行 `stream="yes"` 时才能输出。校验器：`stream="yes"` 而没有 `stream` 行时警告 `STREAM_LINE_MISSING`；有 `stream` 行而首行不是 `yes` 时报错 `STREAM_WITHOUT_ATTR`。
...
1. 取值是**整数秒**，范围 `1s`–`30s`（`HEADER_LAG`）；上限是**经验值**：再大，最短相位（§2.4）就到一分钟，`write` 永远出不了 `peak`。
2. 一个块里只有一个 L：相位行、`clean` 行（§5.2）、`stream` 行（§1.2）都用它；`stream(lag Ns)` 括号里的值**必须**与首行相同（`STREAM_LAG_MISMATCH`）。
3. 块里出现 `peak`、`carryover`、`tail-max` 或 `stream` 行中任一项时，首行**必须**写 `lag`（`LAG_MISSING`）；其它情况**应该**写。没有 `lag` 的 v0.4 块，读者按 v0.3 规则读峰值（最短相位 10 秒、不判 `carryover`）。
4. 换设备时 L 随设备类型变；当轮用的值就是首行写的值，历史轮不重算。

### 2.2 `carryover`

- 相位行的峰值段扩展为 `peak N @Ns[ carryover]`（语法见 §4 的 `peak-part`）。
- 当 `@Ns < L` 时**必须**写 `carryover`；`@Ns ≥ L` 时**不得**写（`CARRYOVER_MISSING` / `CARRYOVER_WRONG`）。含义固定为“峰值出现在本相位开始后 L 秒内”，这是时间事实，不是结论。
- `@Ns` 的口径与相位行本来的一样：从相位开始起算的**在场秒数**（§6：`hidden` / `unfocused` 的秒不计），取整；比较用的就是这个整数，恰好等于 L 时不写。
- 门槛就是 L，不另设窗口、不可单独配置。
- 适用范围：`gen` / `read` / `write` 行、`clean` 行（§5.2-4）、`stream` 行的 `body` 区间（§1.2）。
- 读法应该说明：持续性的心率加速可以延续几十秒（防御反应最长约 80 秒，**有依据**），所以短相位（`write`、`send`）的统计主要是上一相位的余波。

### 2.3 `tail-max`

```
gen: 35s (ttft 9s, body 26s) | hr 68→74 [66–78] peak 78 @30s | cov 97% | tail-max 81 @+3s
```

- 语法 `| tail-max N @+Ns`（§4 的 `tail-max-seg`）。含义：相位结束后第 N 秒时的值，即“余波去了哪里”。
- **值**：相位结束后 L 秒（在场秒）内，用与 `peak` **相同的平滑**（v0.3 §1.4-2 的 5 秒居中中位数）算出的最大值，取整；不要求 `peak` 的“连续 ≥ 3 个样本”条件（窗口太短，**经验值**）。`@+Ns` = 它出现在相位结束后第几秒，`1 ≤ N ≤ L`（`TAILMAX_WINDOW`）。
- **何时写**：本相位有 `peak`、并且这个值 **>** `peak` 时**应该**写；没有 `peak` 的相位**不得**写（`TAILMAX_WITHOUT_PEAK`）；≤ `peak` 时**不得**写（`TAILMAX_NOT_HIGHER`）。稀疏来源没有 `peak`，自然没有 `tail-max`。
- **只用于 `gen`、`read`**：`write` 结束就是发送，块已经生成，后面没有样本（`TAILMAX_PHASE`）。
- 它和下一相位开头带 `carryover` 的峰值多半是同一件事，两处都写是有意的：前一相位行说“余波去了哪里”，后一相位行说“这个峰值来得太早”。

### 2.4 位置换算与最短相位

- 所有位置换算（`read-pos`、`stream` 的 `pos`）使用 `peakAt − L`；结果早于该相位开始时（即 `peak` 带 `carryover`），不输出位置。
- 相位短于 `max(10 s, 2 × L)` 时不输出 `peak`（替换 v0.3 §1.4 的 10 秒；**经验值**；`PEAK_SHORT_PHASE`）。时长按相位行主体写出的秒数。

**兼容**：`carryover` 和 `tail-max` 都在已有行里。v0.3 的校验器把 `carryover` 当成心率段写错（报错），`tail-max` 当成未知段（警告），所以只在 `v="0.4"` 的块里写。LLM 读者多一个词，不影响原意。
...
| 读法需要的量 | 块里的段 | 定义 |
|---|---|---|
| 平均水平 | 相位行 `\| mean 84 (+15%)` | 该行心率样本的算术平均（取整）；括号里是它相对基线的百分比；没有基线时不输出 |
| 持续升高 | 相位行 `\| above +20% 34s` | 该行样本里心率 **>** `基线 × (1 + 阈值)` 的累计秒数；阈值是设置（缺省 20%），写在段里；没有基线时不输出 |
| 离开 | 相位行 `\| away 2:00` | 本相位被剔除的离开时长（§6 的 `hidden`、`unfocused`）；相位的时长与统计本身已不含它 |
| 噪声 | 基线行 `\| noise ±3`（§3） | — |
| 趋势 | `history: … \| read-peak-rel +12% +30% +5%` | 与 `read-peaks` 一一对应：每轮 `read` 的 `peak` 相对**该轮**基线的百分比；没有值写 `·` |
...
规则（2026-09-18 补，可实现性检查；**必须**）：

1. **样本口径**：`mean`、`above` 用的样本 = 该行心率统计用的同一批样本——相位行是在场秒、非摘下的逐秒值（§6、v0.3 §2.1 `wear`）；`clean` 行只用干净秒（§5.2）；`stream` 行只用 `body` 区间。用原始逐秒值，不平滑（`peak` 才平滑）。`mean` 必须落在该行的 `[min–max]` 内（`MEAN_OUT_OF_RANGE`）。
...
- 每段必须写落在哪个相位。
- 空闲判定阈值应该随预计阅读时长伸缩（长回复的阅读期间不应该很快被判成 `idle`）；具体函数由实现决定，但实现应该在 `gates` 行写出这一轮所用的值（§12）。
- **`idle` 只从有内容可读时开始判**：流式时从首字出现起，非流式时从回复写完起；等首字、看思考过程时坐着不动不算离开（2026-09-18，常识规则，依据 `docs/design-review/settings-and-thresholds.md`）。新内容出现（首字、思维链结束、回复写完）和读者的操作一样，都重新开始计“多久没动”。
...
读者什么时候算“不在”、读多久算“太久”、打字停多久算一次停顿，因人而异（有人读得慢、有人边读边想、有人点选项直接发送），不能按某一个人的习惯写死。生产者可以按**本人的行为时长**自动学这些门槛，也可以让用户手动设；无论哪种，都要在块里写明这一轮用的值和来源，模型才知道 `away`、`flag: too-long`、`pauses` 是按什么标准判的。

```
gates(heartlink): idle 1:40 (cal n=12) | too-long 8:00 (user)
gates(heartlink): idle 60s (est) | frozen
```

```
gates-line   = %s"gates(" meta "): " gate *(" | " gate) [" | " %s"frozen"]
gate         = gate-name SP dur SP "(" source ")"
gate-name    = %s"idle" / %s"too-long" / %s"pause"
source       = %s"est" / %s"cal n=" 1*DIGIT / %s"user"
```

`dur` 沿用块里已有的写法（`45s` 或 `m:ss`）；`meta` 是生产者名（v0.3 §2 的 `meta`，同 `haptics(…)`）。

| 门槛 | 管什么 | 缺省（`est`） | 允许范围 | 自动学的依据（非规范，参考实现的做法） |
|---|---|---|---|---|
| `idle` | 多久没有任何操作算 `idle`（§6） | 60 秒 | 60 秒 – 5 分钟 | 干净轮里读回复时相邻两次操作的间隔；长回复按预计阅读时长放宽 |
| `too-long` | 读回复的在场时长超过多少写 `flag: too-long` | 本条字数 ÷ 阅读速度 × 3 | 3 – 30 分钟 | 干净轮里“在场读的时长 ÷ 预计要读多久”的分布 |
| `pause` | 打字时两次输入间隔多久记一次 `pauses` | 5 秒 | 3 – 20 秒 | 干净轮里打字间隔的中位数；程序一次性填进输入框的文字不算 |

1. 这一行在扩展区（`send` 之后、`series` 之前），L0，可选。只写**这一轮用到、并且不是缺省值来源**的门槛也可以；全部是缺省值时可以整行省略。
2. 来源：`est` 还没学到、用缺省；`cal n=N` 由最近 N 个干净轮学到；`user` 用户手动设。`frozen` 表示门槛被锁定（例如研究期间），本轮不再学。
3. **自适应只能由行为时长算出，不得由心率等生理数值推出**——否则门槛会跟着要测的东西一起变。
4. 值必须落在允许范围里；超出范围是错误。学习要有上下限和变化幅度限制（参考实现：每轮最多变 25%），没识别出来的离开不能把门槛越带越宽。
5. 用于学习的行为记录（操作间隔、阅读时长）只留在本机，不进块、不进聊天变量；块里只写门槛值。聊天变量可以加 `bio.gates`（同样只有值和来源）。
6. 首行 `v="0.3"` 的块也**允许**输出这一行：v0.3 规定读者忽略不认识的行（v0.3 §2.3），校验器只给 `LINE_UNKNOWN` 警告。

## 13. 节奏细调：`haptics` 行的 `tuned` 段（2026-09-18 新增）

v0.3 §5.8 允许用户单独改每个档位的参数。改过之后，模型看到的档位名（例如 `profile frenzy`）和实际执行的参数就对不上了，所以生产者必须在 `haptics` 行后面加一段，**只列改过的参数**（和该档位缺省值不同的）：

```
haptics(heartlink): on | cap 100% | profile frenzy | actuators 3 | tuned floor 50%, wave 9s, gap 2s
```

```
tuned-seg    = " | " %s"tuned " tuned-item *(", " tuned-item)
tuned-item   = %s"floor " 1*3DIGIT "%"
             / ( %s"long" / %s"heartbeat" / %s"wave" ) SP secs
             / %s"gap " secs
             / %s"per-reply " %x31-35          ; 1–5
secs         = 1*3DIGIT ["." DIGIT] "s"
```

1. 参数与 v0.3 §5.8 的表一一对应：`floor` 强度下限（0–95%）；`long` / `heartbeat` / `wave` 是这三种持续类模式没写 `ms` 时的时长（1–60 秒）；`gap` 最小间隔（0.2–10 秒）；`per-reply` 每条回复最多执行几个（1–5）。每项最多出现一次。
2. 没有改过任何参数时不写这一段。
3. 首行 `v="0.3"` 的块也可以带这一段：v0.3 §2.3-2 规定已知行的主体后面可以有任意段、读者忽略不认识的段，校验器只给 `SEG_UNKNOWN` 警告。
4. 细调只改“怎么执行”，安全规则不变：强度 0 仍是停止，有风险的输出仍要点名、有时长上限，全部停止仍越过一切。

## 14. 与其他注入方共存（2026-09-19 新增）

同一轮里，预设、卡片脚本、别的扩展也可能在紧贴生成的位置注入内容（例如角色情绪状态、剧情提醒）。为了两边各自更新、互不打架：

1. **块独占一条注入**：实现用自己的注入 id（参考实现用 `bio-context`），不把块并进别人的注入，也不改写别人的注入内容。
2. **位置**：块放在聊天内深度 0（紧贴最新消息之后）。同一深度有多条注入时，SillyTavern 1.18 按注入 id 的字典序排列（不按 `injection_order`，那只管提示词管理器里的条目）；以 `bio-` 开头的 id 会排在 `emo_`、`state_` 这类叙事层注入之前，读者的生理数据在前、角色即将写出的状态离生成点更近——这是推荐顺序，宿主改变排序规则时实现应该维持它。
3. **不串层**：块描述的是**读者**的身体数据。其他注入方（例如情绪预设的“角色情绪暗账”）**不得**把块里的心率、峰值写成角色的情绪或状态；实现也不读取、不解释别的注入方的内容。
4. **标记不冲突**：隐藏规则（正则、显示过滤）只匹配自己的标签——实现只处理 `<bio_context>`、`<bio_act/>`，不处理 HTML 注释或其他自定义标签；反过来也建议其他注入方不要用 `bio_` 开头的标签名。
5. 界面上，悬浮窗各自独立；实现不假设自己是页面上唯一的浮层（点击“面板外”收起时，不拦截、不吞掉别的浮层的点击）。

校验器不检查本节（属于宿主与注入方之间的约定）。

## 15. 模型反问：`<bio_ask/>` 与 `bio:answer`（2026-09-19 新增）

### 15.1 场景与边界

模型在回复正文里，就读者的身体或感受向读者提一个问题（例如"现在这样，你觉得舒服吗？"）；读者用按钮点选或打一句话回答；回答作为读者对自己身体的**自报**，进入下一轮的注入块。按初衷（本仓库 `CLAUDE.md`）："我们做接口，不做理解"——本节只定义问题怎么标记、答案怎么记、下一轮怎么注入，**不**定义模型该不该问、该怎么用答案调整剧情，那是世界书 / 预设的事。

### 15.2 模型侧写法：`<bio_ask/>`（可选约定，同 §5.3）

```
<bio_ask id="a3" kind="feel"/>
<bio_ask id="a4" kind="scale" scale="1-5"/>
<bio_ask id="a5" kind="choice" options="继续,慢一点,停下"/>
```

| 属性 | 必填 | 取值 | 含义 |
|---|---|---|---|
| `id` | 必须 | 1–8 位字母数字 | 本轮内唯一，答案与注入行都靠它对应；由模型或预设按提示写出的短标识，生产者不校验其含义 |
| `kind` | 必须 | `feel` / `scale` / `choice` | `feel` = 开放式感受，读者自由文字作答；`scale` = 数值量表；`choice` = 从候选项里选一个 |
| `scale` | `kind="scale"` 时必须 | `"min-max"`，如 `"1-5"` | 读者作答的整数范围 |
| `options` | `kind="choice"` 时必须 | 逗号分隔的候选文本 | 读者作答必须是其中一项的**原文**，不是序号 |

问题的原文不写进标签属性，取标签**紧邻之前的一段正文**（生产者按启发式截断，非规范；建议：从标签前最近的换行或句末标点开始，最多 40 个字符，超出从头部截断并在末尾加 `…`）——模型只要把标签紧跟在问句之后，不需要另外把问题文字复制进属性。

解析规则与 `<bio_act/>`（§5.3）一致，本节不重复定义、只声明适用：

1. 思维链、代码块、HTML 注释里的 `<bio_ask/>` 不算，生产者必须先去掉这些内容再解析。
2. 生产者必须自己在显示时隐藏 `<bio_ask/>` 标签（不依赖宿主的 HTML 清理）。
3. **每条回复最多 1 个 `<bio_ask/>`**——与 `<bio_act/>` 不同，问答一次只问一件事；多出的（去掉思维链等之后仍剩 ≥ 2 个）只解析出现顺序里的第一个，其余丢弃（诊断码 `ASK_TOO_MANY`，供以后 validator 使用）。
4. 实现**不得**替读者作答、不得由心率等生理信号推断答案填入——`value` 只能来自读者的显式操作（§15.3）。

具体 UI（按钮、输入框、还是别的呈现）不规范，由实现自行设计；本节只规定接口。

### 15.3 事件与总线

```js
tbc.on('bio:ask', fn);
// 生产者解析到新的 <bio_ask/> 时触发
// detail: { id, kind, question, options?: string[], scale?: [number, number], t }

tbc.answer({ ask: 'a3', value: '有点太快了', t: Date.now() });
// 读者作答时调用；value 按 kind：feel = 字符串（生产者应设长度上限，建议 ≤ 100 字，
// 防止占满上下文，具体上限不规范）；scale = scale 范围内的整数；choice = options 中的原文
tbc.on('bio:answer', fn);   // detail 同上，加 source: 'reader'

tbc.pendingAsk();
// 只读：当前待回答的问题（{ id, kind, question, … } 或 null）；
// 读者发送下一条消息（不论答没答）时清空，不留到更下一轮
```

- 生产者：负责解析标签、隐藏标签、维护 `pendingAsk()`、在读者作答时调用注册的应答接口生成 `bio:answer`；具体触发 `tbc.answer()` 的 UI 由生产者自建（按钮、输入框、语音转文字皆可），也可以由第三方脚本代为调用。
- 一轮里模型没有提问、读者也没有待回答的问题时，`bio:ask` 不触发，`pendingAsk()` 为 `null`。
- `bio:answer` 只记录，不解释、不触发任何设备动作或新的生成——是否要顺着回答调整剧情，由用户自己的模型在读到下一轮的 `ask` 行之后决定。

### 15.4 块里的 `ask` 行（扩展区，L0）

```
ask(a3): "现在这样，你觉得舒服吗？" -> 有点太快了 @read +12s
ask(a4): "1 到 5，你现在有多兴奋？" -> 4 @write +3s
ask(a5): "继续、慢一点，还是停下？" -> 慢一点 @send +0s
ask(a6): "还撑得住吗？" -> unanswered
ask: n/a
```

语法（完整版留待 `schema/block.abnf` 实现时补full ABNF，此处先定文字与示例）：

```
ask-line     = %s"ask(" id "): " ( quoted-q " -> " ( answered / %s"unanswered" ) / %s"n/a" )
id           = 1*8(ALPHA / DIGIT)
quoted-q     = DQUOTE 1*40(%x20-21 / %x23-7E) ["…"] DQUOTE   ; 不含双引号；超 40 字截断并加省略号
answered     = value " @" phase " +" 1*DIGIT "s"
value        = 1*(%x20-7E)         ; feel/choice 是原文；scale 是十进制整数
phase        = "gen" / "read" / "write" / "send"
```

| 段 / 写法 | 含义 |
|---|---|
| `ask(<id>): "<问题>" -> <回答> @<相位> +<Ns>` | 该问题在哪个相位、相位开始后几秒被回答；相位与 `@Ns` 的写法沿用 §5.12.2 `feedback` 行的 `event-seg` |
| `ask(<id>): "<问题>" -> unanswered` | 模型问了，但到本轮发送时读者仍未作答；**不追问**——实现不得因未回答自动生成新的 `<bio_ask/>` 或重复注入同一问题，是否再问、怎么问由模型自己下一轮决定 |
| `ask: n/a` | 本轮没有新问题、也没有待回答或刚回答的问题；可以整行省略 |

规则：

1. **一轮最多一行 `ask`**（即使 `<bio_ask/>` 一条回复最多 1 个，问题也只可能有一个待处理）。
2. 问题文本、回答文本都不做任何解读或转述，原样（截断后）写出；`feel` 类回答如含 `|`、双引号、换行，生产者应转义或替换为空格，避免破坏行结构（具体转义规则留待 validator 实现时定）。
3. `unanswered` 之后，若读者始终不答，下一轮发送时 `pendingAsk()` 已按 §15.3 清空，不再输出该 `id` 的 `ask` 行（不写"仍未回答"这类累积状态）。
4. `ask` 行与 `feedback` 行（§5.12.2）是两类独立信号：`feedback` 记录读者对**设备**的操作（停/强/弱/换挡），`ask`/`answer` 记录读者对**文字提问**的自报；两者互不影响、互不合并，行的先后顺序不做规定。
5. 隐私：回答内容属于读者对自己身体/感受的主观陈述，只应作为提示词交给用户自己配置的模型；聊天记录导出会带出原文，与 `bio.lastSignal`（v0.2 §8）一样，生产者应提供关闭这项记录的开关。

### 15.5 变量

- `bio.pendingAsk`：`{ id, kind, question, options?, scale? } | null`，同 `tbc.pendingAsk()`。
- `bio.lastAnswer`：`{ id, kind, question, value, t, phase, offsetS } | null`，最近一次作答，供卡片脚本读；发送后清空。
- Schema：`schema/bio-ask.schema.json`（以后实现校验器时补）。

### 15.6 校验要点（validator 以后实现，本次只定文字）

- `id` 在同一块内唯一；`quoted-q` 截断到 40 字符（超出必须带 `…`）；`kind="scale"` 的 `value` 必须是 `scale` 范围内的整数，`kind="choice"` 的 `value` 必须逐字等于 `options` 中的一项，`kind="feel"` 不做取值校验（自由文本）；`unanswered` 不得带 `value` 或 `@相位`；每块最多一行 `ask`；一条回复最多 1 个 `<bio_ask/>`（多出的记 `ASK_TOO_MANY`，不影响已解析的第一个）；`ask` 行引用的 `id` 必须能在本轮或上一轮的 `<bio_ask/>` 里找到对应问题，否则报 `ASK_UNKNOWN_ID`。

...
| `mean N%` | 这些秒里发出的强度按时间加权的平均，`强度 × 100` 取整（1–100） |
...
3. **强度**：某个执行器在某一秒里的强度 = 该秒内按时间加权的平均（帧是阶梯函数）；该秒的强度 = 各执行器里的**最大值**（多台设备不因为取平均而变弱）。`mean` = 计入的那些秒的强度的算术平均。取的是**发出的值**（抬高档位下限、裁到用户上限之后的值，v0.3 §5.8），不是模型写的原值。
...
```
pressure(intiface:0:1, 100ms): read +0→+12 peak +18 @22s | write +2
button(intiface:0:1, event): read @41s ×1
```

1. 括号里第一项写**执行器 id**（`<bio_act target="…"/>` 能用的那个值，按 v0.3 §4.4 的标识规则，可以含 `:`），第二项照常是 cadence；总线上推样本时 `device` 写同一个 id。
...
### 8.5 停止失败与断线后状态未知（2026-09-19 新增）

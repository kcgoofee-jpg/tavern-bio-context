...
5. **设备自带模式**：执行器可在 `nativePatterns` 里把抽象模式映射到设备自带模式编号；没有映射的模式按强度帧执行。自带模式默认要求**强度 0 能立即停下**。停不下来的自带模式（例：有产品的“失控模式”启动后要等计时结束才停）只有同时满足下列条件才能登记（2026-09-18 用户定：读者可以自己拔出或按设备按钮停止）：
   ...
   - 列表写法的 `nativePatterns`、模型点名自带模式（`pattern="native"`）与这些条件的可检查形式见 [v0.4 草案 §8](spec-v0.4-draft-zh.md)。
...
- 流式显示、心率滞后、基线年龄与噪声、派生事实段、away 相对写法、世界书扫描缺省关闭、执行器电量与连接、设备自带模式直通：见 [v0.4 草案](spec-v0.4-draft-zh.md)。

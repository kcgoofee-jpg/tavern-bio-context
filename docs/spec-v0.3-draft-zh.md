...
3. 有 `perceiver` 且当前模式不是 `backstage` 时，首行加属性 `perceiver="小影"`（多个用 `、` 连接，去掉引号与尖括号）。它是元数据，不是结论。
...
- 每条回复最多 3 个（用户可改，最多 5，见 §5.8），多出的丢弃；按出现顺序排队，同一执行器之间至少隔 `minIntervalMs`。
...
3. **频率上限**：同一执行器两次触发间隔 ≥ 它的 `minIntervalMs`（手环缺省 10 秒）与档位缺省值中的较大者；单条回复次数按 §5.8（缺省 3，最多 5）。
...
用到玩具的用户通常想要高触发、高功率；也有人想慢慢来。所以协议不设“亲密闸门”，而是给两个**档位**，让用户第一次使用时选，之后随时改；每个参数也都能单独改。难以统一测准的数值（时长、间隔、下限）一律做成**有缺省值的用户设置**。

| 参数 | 慢热 `slow-burn`（缺省） | 狂暴 `frenzy` | 说明 |
|---|---|---|---|
| `floor` 强度下限 | 0 | 0.4 | 非 0 强度按 `floor + (1 − floor) × 强度` 抬高；0 仍是停止 |
| `defaultMs.long` | 1500 | 5000 | 持续类模式没写 `ms` 时的时长 |
| `defaultMs.heartbeat` | 2700 | 5400 | 同上 |
| `defaultMs.wave` | 3000 | 6000 | 同上 |
| `minIntervalMs` | 1500 | 800 | 缺省最小间隔；执行器声明的更大时取更大者 |
| `maxPerReply` | 3 | 5 | 每条回复最多执行几个（上限 5） |
| 用户强度上限 | 0.6 | 0.6 | 抬高之后再裁到上限（§5.4-2） |
...
- 注入侧同理：阈值类参数（手动基线、覆盖率门槛 70%、“噪声”范围 10%、“明确反应” 20% 等）应作为有缺省值的设置暴露给用户，块内照常只记录数值。
...
5. **设备自带模式**：执行器可在 `nativePatterns` 里把抽象模式映射到设备自带模式编号；没有映射的模式按强度帧执行。
6. **有风险的输出要点名**：`Temperature`、`Estim`、`Spray` 不属于 `output="*"`（包括不写 `output`）的范围；只有写明这个输出类型才会驱动，否则返回 `unsupported`（参考实现 `RISKY_OUTPUTS`）。
7. 字节格式、档位数、握手、双芯片（一个型号对应两个蓝牙模块）等属于驱动实现，不进协议（同一系列不同型号就不一样；社区经验是先抓包，不要盲发网上的指令）。

### 5.10 MCP 桥（非规范）

社区已有把玩具 / 可穿戴接给 AI 客户端的 MCP 服务（调研见本地 `references/research/2026-09-17-glama-mcp.md`）。实现要做 TBC ↔ MCP 桥时：

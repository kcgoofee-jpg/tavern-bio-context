...
- 每条回复的动作数不设实质上限（参考实现取 50 只防失控，用户可改，见 §5.8；2026-09-19 起），**按去掉上述内容后剩下的标签计算**，多出的丢弃；按出现顺序排队，同一执行器之间至少隔 `minIntervalMs`。读者开始新一轮（发送 / 滑动 / 重新生成）时，上一条回复里**还没开始**的排队动作作废（结局记 `cut`），正在执行的走完——防止动作多、时长长的回复串到下一轮（2026-09-19）。

...
3. 有 `perceiver` 且当前模式不是 `backstage` 时，首行加属性 `perceiver="小影"`（多个用 `、` 连接）。每个名字按 §4.4 的自由文本规则清洗，并去掉 `"` 与 `、`。它是元数据，不是结论。
...
- 每条回复最多 3 个（用户可改，最多 5，见 §5.8），**按去掉上述内容后剩下的标签计算**，多出的丢弃；按出现顺序排队，同一执行器之间至少隔 `minIntervalMs`。
...
- 流式显示、心率滞后、基线年龄与噪声、派生事实段、away 相对写法、世界书扫描缺省关闭：见 [v0.4 草案](spec-v0.4-draft-zh.md)。

...
用到玩具的用户通常想要高触发、高功率；也有人想慢慢来。所以协议不设“亲密闸门”，而是给两个**档位**，让用户第一次使用时选，之后随时改（v0.3 定义四档：慢热、持久、狂暴、极限）；每个参数也都能单独改。难以统一测准的数值（时长、间隔、下限）一律做成**有缺省值的用户设置**。
...
| `maxPerReply` | 3 | 3 | 5 | 5 | 每条回复最多执行几个（上限 5） |

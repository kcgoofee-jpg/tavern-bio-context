...
| `TOY_STOP_FAILED` | 停止指令重试后设备仍没有应答（§5.9-9） | 拔出或按设备按钮；检查连接 |
| `TOY_MAYBE_RUNNING` | 设备断线且 `stopsOnDisconnect` 为假，可能仍在动（§5.9-8） | 重连后实现会先发停止；必要时手动停 |
| `TOY_EXCLUSIVE_BUSY` | 设备只接受一个连接，已被别的程序占着（§5.9-7） | 先断开官方 App 或其它页面 |
| `TOY_HANDSHAKE_TIMEOUT` | 连上后握手没在规定时间内完成 | 断开重连；驱动的握手事实可能不对 |
...
| `BASELINE_METHOD_DEPRECATED`、`BASELINE_AGE_MISSING`、`BASELINE_MANUAL_INFO`、`LAG_MISSING`、`CARRYOVER_*`、`TAILMAX_*`、`STREAM_*`、`MEAN_*`、`ABOVE_*`、`DERIVED_WITHOUT_BASELINE`、`AWAY_SEG_MISMATCH`、`HISTORY_REL_COUNT`、`ACT_*`、`CLEAN_*`、`TOY_SENSOR_*`、`ACTUATOR_*`、`NATIVE_*` | v0.4 草案 | 警告 |

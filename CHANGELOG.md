...
- 仓库工程：新增 GitHub Actions CI（push / PR 时 `npm ci && npm test`，Node LTS）；package.json 的 `version` 由 `0.3.0-draft` 改为 `0.4.0-draft`，对齐 README 所列最新草案版本（协议版本仍以块首行 `v=` 为准，package.json 不发布、无协议语义）。
- v0.4 §15（新增）：模型反问 `<bio_ask/>` 与读者作答 `bio:answer`——三种问法（`feel` 开放式 / `scale` 量表 / `choice` 多选一）、每条回复最多 1 个问题、注入行 `ask(<id>): "<问题>" -> <回答> @<相位> +<Ns>`（未答写 `unanswered`，不追问）、`pendingAsk()` / `bio.pendingAsk` / `bio.lastAnswer`。只定接口：问题怎么标记、答案怎么记、怎么注入；理解与调整剧情仍由用户自己的模型决定。依据：TODO 待办"模型反问用户，回答直接注入"，先在协议里定问答事件和注入行。
- v0.4 §14（新增）：与其他注入方共存——块独占一条注入、深度 0、同深度按 id 字典序（`bio-` 在叙事层注入之前）、其他注入方不得把读者心率写成角色情绪、隐藏规则只匹配自己的标签、浮层不吞别人的点击。依据：情绪功能预设 v4 与 heartlink 并行使用的冲突分析。
- v0.3 §5 / §5.8：每条回复的动作数不再设上限（四档 `maxPerReply` 统一为 50，只防失控）。依据：参考实现 heartlink 按用户要求取消玩具部分的软性限制（玩具功能暂缓、由用户自己把握）；“全部停止”、断线即停、有风险的输出必须点名与其硬时长上限不变。
- v0.3 §5.3：新一轮开始时，上一条回复里还没开始的排队动作作废（记 `cut`），正在执行的走完。依据：取消上限后长队列会串到下一轮（heartlink 逻辑审查 F-103）。
## [Unreleased] — 0.4 草案
...
- §1.3 规则加一条（2026-09-19，heartlink 0.21.2 F-097 实测反馈）：打字打到一半又全部删掉的段落不算 `write` 的起点——发送框删空（长度回到 0）之前的打字作废，`write` 从最后一段没被删空的打字开始；删空之后到再次打字之间仍算 `read`；发送时输入框是空的则没有 `write` 相位。
